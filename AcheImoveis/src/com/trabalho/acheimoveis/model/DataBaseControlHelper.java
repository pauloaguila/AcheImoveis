...
/*
    public Dao<Book, Integer> getModelBookDAO() {
        if (null == modelBookDAO) {
            try {
                modelBookDAO = getDao(Book.class);
            } catch(java.sql.SQLException e) {
                e.printStackTrace();
            }
    ...
    public Dao<Verse, Integer> getModelVerseDAO() {
        if (null == modelVerseDAO) {
            try {
                modelVerseDAO = getDao(Verse.class);
            } catch(java.sql.SQLException e) {
                e.printStackTrace();
            }
        }
        return modelVerseDAO;
    }

    public Dao<Favorite, Integer> getModelFavoriteDAO() {
        if (null == modelFavoriteDAO) {
            try {
                modelFavoriteDAO = getDao(Favorite.class);
            } catch(java.sql.SQLException e) {
                e.printStackTrace();
            }
        }
        return modelFavoriteDAO;
    }
*/
}
