...
public class MemoryHelper {
	...
	public static boolean hasSpaceAvailable(Context applicationContext) {
		...
		File databasePath = applicationContext
		...
		for (int i = 0; i < assetsContent.length; ++i) {
			...
			if (!MemoryHelper.databasesSize.containsKey(assetsContent[i])) {
				long assetDBLength = MemoryHelper.getDatabaseSize(
						applicationContext, null, assetsContent[i]);
		...
		if (databases == null || databases.length == 0) {
			...
			allMemoryNeeded = MemoryHelper.databasesSize
		...
		// Database already exists in user data
			...
			if (Constants.DATABASE_OLD_VERSION < Constants.DATABASE_NEW_VERSION) {
				...
				long currentDatabaseSize = MemoryHelper.getDatabaseSize(
	...
	public static boolean hasSpaceAvailableForDatabase(
			Context applicationContext, String databasePath, String database) {
		long databaseSize = getDatabaseSize(applicationContext, new File(
				databasePath), database);
		return MemoryHelper.getAvailableMemory() > databaseSize;
	}

	private static long getDatabaseSize(Context applicationContext,
			File databasePath, String database) {
		...
		if (databasePath != null) {
			...
			try {
				AssetFileDescriptor descriptor = applicationContext.getAssets()
						.openFd(database);
